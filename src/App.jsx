...
// =====================================================================
// Fetches and parses the four Lahman CSV files once, then buildPlayerSeasons
// re-filters cheaply whenever settings (startYear / endYear / minPA) change.
//
// CACHING STRATEGY (two layers):
//   Layer A — HTTP Cache-Control headers (vite.config.js) make normal F5 refreshes
//             return 304 Not Modified so the browser never re-downloads the CSVs.
//   Layer B — sessionStorage stores the parsed result so hard-refreshes within the
//             same browser session skip the 24 MB download entirely.
//             (Best-effort: silently fails if quota is exceeded — Layer A still helps.)

// Bump when the Lahman CSV files are updated to invalidate the session cache.
const LAHMAN_CACHE_KEY = "lahman-v2025-01";
...
function useLahmanData() {
  ...
  useEffect(() => {
    // Layer B: try sessionStorage first
    try {
      const hit = sessionStorage.getItem(LAHMAN_CACHE_KEY);
      if (hit) {
        setRaw(JSON.parse(hit));
        setCsvLoading(false);
        return;
      }
    } catch {}
    ...
    Promise.all([
      ...
      .then(([p, b, f, pit]) => {
        ...
        setRaw(parsed);
        setCsvLoading(false);
        try { sessionStorage.setItem(LAHMAN_CACHE_KEY, JSON.stringify(parsed)); } catch {}
      })
...
// Loads public/headshots.json (name → URL map generated by scripts/fetch_headshots.py).
// Silently no-ops if the file doesn't exist yet so the app works without headshots.
...
// League logo image sources — tries local file first, then Wikimedia, then badge fallback.
// To self-host: download the Wikimedia URLs below and save to /public/al-logo.png & nl-logo.png.
const LEAGUE_LOGO_SRCS = {
  AL: [
    "/al-logo.png",
    "https://upload.wikimedia.org/wikipedia/en/thumb/5/54/American_League_logo.svg/1280px-American_League_logo.svg.png",
  ],
  NL: [
    "/nl-logo.png",
    "https://upload.wikimedia.org/wikipedia/en/thumb/d/d4/MLB_National_League_logo.svg/1280px-MLB_National_League_logo.svg.png",
  ],
};

function LeagueLogo({ league, label, size = 44 }) {
  const [stage, setStage] = useState(0); // 0=local file, 1=wikimedia, 2=text badge
  const bg   = league === "AL" ? "#003087" : "#C6011F";
  const r    = Math.round(size * 0.18);
  const srcs = LEAGUE_LOGO_SRCS[league] ?? [];
  return (
    <div style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:"4px",minWidth:"70px" }}>
      <div style={{ width:size,height:size,borderRadius:r,background:bg,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,overflow:"hidden" }}>
        {stage >= srcs.length ? (
          <span style={{ color:"#fff",fontSize:size*0.37,fontWeight:900,letterSpacing:"-0.02em" }}>{league}</span>
        ) : (
          <img
            src={srcs[stage]}
            alt={league}
...
// Renders a player headshot circle. Returns null (no space) if no URL is available or image fails.
function PlayerPhoto({ name, headshots, size = 40 }) {
...
// -- Result Card --
function ResultCard({ playerName, year, team, score, statLabel, percentile, onClick, headshots = {} }) {
...
function Top5Panel({ validAnswers, scoringStatKey, scoringStatLabel, lowerIsBetter, headshots = {} }) {
...
// -- Puzzle Row --
function PuzzleRow({ row, rowIndex, scoringStat, submission, allRows, playerSeasons, pitcherSeasons, isPitcherPuzzle, onClickAdd, retryMode, wrongRowAttempts, headshots = {} }) {
  ...
  if (submission?.correct) {
    return (
      <div style={{ marginBottom:"8px" }}>
        <ResultCard playerName={submission.playerName} year={submission.year} team={submission.team} score={submission.score} statLabel={scoringStat.label} percentile={percentile} onClick={()=>setShowStats(!showStats)} headshots={headshots} />
        {showStats && matchedSeason && <StatsPanel ps={matchedSeason} />}
        {showStats && <Top5Panel validAnswers={row.validAnswers} scoringStatKey={scoringStat.key} scoringStatLabel={scoringStat.label} lowerIsBetter={scoringStat.lowerIsBetter} headshots={headshots} />}
      </div>
    );
  }
        ...
        {activePage === "statpad" ? (
          <>
            ...
            {puzzle.rows.map((row,i) => (
              <PuzzleRow key={`${puzzle.id}-${i}`} row={row} rowIndex={i} scoringStat={puzzle.scoringStat} submission={submissions[i]} allRows={puzzle.rows} playerSeasons={playerSeasons} pitcherSeasons={pitcherSeasons} isPitcherPuzzle={puzzle.isPitcherPuzzle} onClickAdd={setActiveRow} retryMode={retryMode} wrongRowAttempts={wrongAttempts[i]} headshots={headshots} />
            ))}
